...
export default function SidebarDrones({ drones }: SidebarDronesProps) {
  return (
    <div className="sidebar-section">
      <h2 style={{ display: 'flex', alignItems: 'center', gap: '8px', color: 'var(--neon-cyan)', marginBottom: '16px' }}>
              ...
              <div key={drone.id} className="queue-card">
                <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '8px' }}>
                  ...
                  <span className={`status-badge`} style={{
                      ...
                      color: drone.status === 'IDLE' ? 'var(--neon-cyan)' : 'var(--neon-red)'
                  }}>
                    {drone.status}
                  </span>
                </div>
                ...
                <div style={{ display: 'flex', alignItems: 'center', gap: '8px', fontSize: '0.9rem', color: 'var(--text-secondary)' }}>
                  <BatteryIcon size={16} color={batteryPct > 25 ? "var(--neon-cyan)" : "var(--neon-red)"} />
